...
class BasicAPIStructureGrader(TaskGrader):
    ...
    def grade(self, schema_json: str, validation_result: ValidationResult) -> TaskGrade:
        ...
        try:
            ...
            strict_score = self._strict_unit_interval(0.0)
            return TaskGrade(
        ...
        if len(present_fields) == len(required_fields):
            ...
            feedback_parts.append(
                f"✗ Missing fields: {', '.join([f for f in required_fields if f not in present_fields])}"
            )
        ...
        endpoint_score = (
            min(endpoint_count / 2, 1.0) * 0.25
        )  # Full points for 2+ endpoints
        ...
        if endpoint_count >= 2:
            feedback_parts.append(
                f"✓ Good endpoint coverage ({endpoint_count} endpoints)"
            )
        else:
            feedback_parts.append(
                f"✗ Need more endpoints (found {endpoint_count}, need 2+)"
            )
            ...
            for method in ["get", "post", "put", "patch", "delete"]:
                if method in path_item:
                    ...
                    if method == "get":
                        ...
                        correct_methods += (
                            1  # PUT, PATCH, DELETE are generally correct if present
                        )
        ...
        if total_operations > 0:
            feedback_parts.append(
                f"✓ HTTP methods: {correct_methods}/{total_operations} correct"
            )
        else:
        ...
        if len(critical_errors) == 0:
            ...
            feedback_parts.append(
                f"✗ {len(critical_errors)} critical errors need fixing"
            )

        # Determine pass/fail (threshold: 0.6)
        score = self._strict_unit_interval(score)
        passed = score >= 0.6
...
class SecurityAuthenticationGrader(TaskGrader):
    ...
    def grade(self, schema_json: str, validation_result: ValidationResult) -> TaskGrade:
        ...
        if security_schemes:
            scheme_score = 0.4
            feedback_parts.append(
                f"✓ Security schemes defined ({len(security_schemes)} schemes)"
            )
        else:
            scheme_score = 0.0
        ...
        if valid_schemes == len(security_schemes) and security_schemes:
            ...
            feedback_parts.append(
                f"✗ {len(security_schemes) - valid_schemes} invalid scheme types"
            )
        ...
        if protected_operations == total_operations and total_operations > 0:
            ...
            feedback_parts.append(
                f"✗ {total_operations - protected_operations} unprotected endpoints"
            )
        else:
        ...
        score = self._strict_unit_interval(score)
        passed = score >= 0.7
        feedback = f"Security & Authentication: {score:.1%} | " + " | ".join(
            feedback_parts
        )
...
class AdvancedBestPracticesGrader(TaskGrader):
    ...
    def grade(self, schema_json: str, validation_result: ValidationResult) -> TaskGrade:
        ...
        if documented_operations == total_operations and total_operations > 0:
            ...
            feedback_parts.append(
                f"✗ {total_operations - documented_operations} operations lack documentation"
            )
        ...
        if operations_with_errors == total_operations and total_operations > 0:
            ...
            feedback_parts.append(
                f"✗ {total_operations - operations_with_errors} operations missing error responses"
            )
        ...
        details["schemas"] = {"count": len(schemas), "score": schema_score}

        if len(schemas) >= 3:
            feedback_parts.append(f"✓ Good schema definitions ({len(schemas)} schemas)")
        else:
            feedback_parts.append(
                f"✗ Need more schema definitions (found {len(schemas)}, recommended 3+)"
            )
        ...
        score = self._strict_unit_interval(score)
        passed = score >= 0.8
        feedback = f"Advanced Best Practices: {score:.1%} | " + " | ".join(
            feedback_parts
        )
...
class TaskGradingSystem:
    ...
    def grade_all_tasks(
        self, schema_json: str, validation_result: ValidationResult
    ) -> Dict[str, Any]:
        """
